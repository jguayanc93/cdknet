...
async function programador_permisos(req,res,next) {
    try{
...
function ver_permisos(resolve,reject,payload,tipo,req,next){
    ...
    for(const access of Object.keys(permisos["programador"])){
        if(permisos["programador"][access].includes(parseInt(grupo))){
...
module.exports={programador_permisos}