...
async function factura_permisos(req,res,next) {
    try{
...
function ver_permisos(resolve,reject,payload,tipo,req,next){
    ...
    for(const access of Object.keys(permisos["factura"])){
        if(permisos["factura"][access].includes(parseInt(grupo))){
...
module.exports={factura_permisos}