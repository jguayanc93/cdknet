...
async function pedi_permisos(req,res,next) {
    try{
...
function ver_permisos(resolve,reject,payload,tipo,req,next){
    ...
    for(const access of Object.keys(permisos["pedido"])){
        if(permisos["pedido"][access].includes(parseInt(grupo))){
